    ...
    <div className={`relative flex items-center ${isSender ? 'flex-row-reverse' : ''}`}>
      ...
      <Image 
        ...
        <div className={isSender ? 'mr-3' : 'ml-3'}>
          ...
          <div className={`text-sm text-white py-1.5 mt-3 px-3 rounded-2xl ${isSender ? 
          'rounded-tr-none bg-primary' 
        : 'rounded-tl-none bg-border'}`}>{message.text}
        </div>

          <div className={`text-xs opacity-30 block mt-1.5 ${isSender 
